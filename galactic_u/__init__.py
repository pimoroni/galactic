...
def get_board():
  if model == "indoor":
    import galactic.boards.indoor as board
  if model == "grow":
    import galactic.boards.grow as board
  if model == "weather":
    import galactic.boards.weather as board
  if model == "urban":
    import galactic.boards.urban as board
  return board
...
import galactic.helpers as helpers
...
def upload_readings():
  ...
  exec(f"import galactic.destinations.{destination}")
  destination_module = sys.modules[f"galactic.destinations.{destination}"]
